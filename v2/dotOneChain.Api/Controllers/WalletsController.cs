...
public class WalletsController(ICryptoService crypto,MongoContext mongo) : ControllerBase
{
    ...
    {
        var holdings = await mongo.Holdings.Find(h => h.OwnerAddress == address && h.Balance > 0).ToListAsync(ct);
        ...
        var items = holdings.Select(h => new {
            ...
            {
                t.Name, t.Description, t.CurrentObjectCid, t.CurrentVersion, t.MaxSupply, Circulating = t.TotalMinted - t.TotalBurned
            } : null
        });
    ...
    public async Task<IActionResult> History(string address, [FromQuery] int page = 1, [FromQuery] int pageSize = 50, CancellationToken ct = default)
    {
        ...
        var list = await mongo.Transactions.Find(filter).SortByDescending(t => t.CreatedAt).Skip((page - 1) * pageSize).Limit(pageSize).ToListAsync(ct);
