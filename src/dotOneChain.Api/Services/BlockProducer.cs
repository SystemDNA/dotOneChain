...
public class BlockProducer : BackgroundService
{
    ...
    public BlockProducer(IServiceScopeFactory scopeFactory, IMempool mempool, IHashService hash, IMerkleService merkle, ICryptoService crypto, Microsoft.Extensions.Options.IOptions<AuthorityOptions> auth)
    {
    ...
    private async Task ProduceAsync(CancellationToken ct)
    {
        ...
        // Helper to validate then apply a group
        async Task<List<Tx1155>> ValidateApplyAsync(List<Tx1155> batch)
        {
            ...
            if (keep.Count > 0)
                await ledger.ApplyAsync(keep, ct);  // updates holdings/tokens AND inserts txs
            return keep;
        }

        // 2) Apply in order so later validations see earlier effects
        var appliedMints = await ValidateApplyAsync(mints);     // balances credited
        var appliedTransfers = await ValidateApplyAsync(transfers); // sees minted balances
        var appliedBurns = await ValidateApplyAsync(burns);     // sees post-transfer balances

        // If nothing applied, don’t write a block
        var appliedAll = appliedMints.Count + appliedTransfers.Count + appliedBurns.Count;
        if (appliedAll == 0) return;

        // 3) Build the block from the APPLIED txs only
        var applied = appliedMints.Concat(appliedTransfers).Concat(appliedBurns).ToList();

        var prev = await mongo.Blocks.Find(FilterDefinition<Block>.Empty)
