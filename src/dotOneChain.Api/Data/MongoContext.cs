...
public class MongoContext
{
    ...
    public async Task EnsureIndexesAsync(CancellationToken ct = default)
    {
        ...
        await Wallets.Indexes.CreateOneAsync(
                    new CreateIndexModel<Wallet>(
                        Builders<Wallet>.IndexKeys.Ascending(w => w.Label),
                        new CreateIndexOptions { Name = "idx_wallets_label" }),
                    cancellationToken: ct);


            await Transactions.Indexes.CreateOneAsync(
                        new CreateIndexModel<Tx1155>(
                            Builders<Tx1155>.IndexKeys.Ascending(t => t.FromAddress).Ascending(t => t.CreatedAt),
                            new CreateIndexOptions { Name = "idx_tx_from_time" }), cancellationToken: ct);

            await Transactions.Indexes.CreateOneAsync(
                new CreateIndexModel<Tx1155>(
                    Builders<Tx1155>.IndexKeys.Ascending(t => t.ToAddress).Ascending(t => t.CreatedAt),
                    new CreateIndexOptions { Name = "idx_tx_to_time" }), cancellationToken: ct);
    }
}
